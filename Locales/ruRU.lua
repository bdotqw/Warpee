...
local STRINGS = {
  ...
  ["Reads the code in the field and asks before replacing your list; the list you had is kept as a profile, so it can be brought back"] = "Читает код из поля и спрашивает перед заменой вашего списка; прежний список сохраняется как профиль, так что его можно вернуть.",
